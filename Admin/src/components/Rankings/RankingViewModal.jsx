...
const RankingViewModal = ({ openModal, handleClose, data }) => {

    const [idRanking, setIdRanking] = useState("");
    const [infoRankingPos, setInfoRankingPos] = useState([]);

    console.log(data);

    useEffect(() => {
        setIdRanking(data?.id);
        try {
            getInfoTorneioById({ id: data.id }).then((response) => {
                console.log(response.data.infoTorneio);
                setInfoRankingPos(response.data.infoTorneio);
            });
        } catch (error) {
    ...
    return (
        ...
        >
            ...
            <DialogContent>
                {renderPlayers(infoRankingPos.sort((a, b) => a.posicao - b.posicao))}
            </DialogContent>
            <DialogActions>
...
export default RankingViewModal;